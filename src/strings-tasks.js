...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((sum, current) => sum + current.charCodeAt(), 0);
}
...
function isPalindrome(str) {
  const strLowReg = str.toLowerCase().replace(/\W/g, '');
  return strLowReg === strLowReg.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  const normABC = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const rot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    let letter = str[i];
    if (normABC.includes(str[i])) {
      letter = rot13[normABC.indexOf(str[i])];
    }
    result += letter;
  }
  return result;
